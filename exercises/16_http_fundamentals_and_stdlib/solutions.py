"""Solutions: Chapter 16 - HTTP Fundamentals and the Standard Library."""
...
def parse_target(target: str) -> ParsedTarget:
    ...
    if parts.scheme or parts.netloc or parts.fragment:
        raise ValueError("target must be a local path without a fragment")

    if parts.path == "/health":
        if parts.query:
            raise ValueError("health does not accept query fields")
        return ParsedTarget("health", None, None)
    ...
    if parts.path == "/tasks":
        ...
        return ParsedTarget("tasks", None, completed)

    prefix = "/tasks/"
    segment_text = parts.path[len(prefix) :]
    if parts.path.startswith(prefix) and "/" not in segment_text:
        if parts.query:
            raise ValueError("one task does not accept query fields")
        segment = unquote(segment_text)
        if not segment.isdecimal() or int(segment) <= 0:
            raise ValueError("task ID must be a positive integer")
        return ParsedTarget("task", int(segment), None)

    return ParsedTarget(None, None, None)
...
def dispatch_request(request: HttpRequest) -> HttpResponse:
    try:
        ...
        return _error(HTTPStatus.BAD_REQUEST, "invalid_target", str(error))
    ...
    if target.route == "task":
        ...
        task = next((item for item in TASKS if item["id"] == target.task_id), None)
        if task is None:
            return _error(
                HTTPStatus.NOT_FOUND,
                "not_found",
                f"task {target.task_id} was not found",
            )
        return json_response(HTTPStatus.OK, task)

    if method == "GET":
        tasks = [
            task
            for task in TASKS
            if target.completed is None or task["completed"] is target.completed
        ]
    ...
    if (
        ...
        return _error(
            HTTPStatus.BAD_REQUEST,
            "invalid_request",
            "request must contain exactly one non-empty string title",
        )
    return json_response(
        HTTPStatus.CREATED,
        {
