...
imports a server framework or HTTP client library. The exact classes, modules,
traits, interfaces, or functions used to express these boundaries are left to
the target language.

All clients interoperate with all servers. A standard-library client is not
paired only with the standard-library server, and a framework's preferred
client is not allowed to rely on private behavior from that framework.
...
The SQLite implementation uses the platform's ordinary SQLite library and
parameterized SQL. It stores tasks in one table with:
...
- the repository abstraction may be a protocol, interface, trait, structural
  type, or set of injected callables;
- domain values may use records, data classes, structs, or another clear
  immutable/read-only representation;
- route registration and dependency injection should follow the chosen
  framework's conventions;
- synchronous implementations are sufficient;
